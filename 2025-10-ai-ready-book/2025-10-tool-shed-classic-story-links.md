Excellent.
Below is the **Tool Shed Sharing Program API Story** rewritten under your new **Navigational Completeness Rule**, which ensures:

1. Every resource includes a `showHome` action returning **Home**.
2. Every item-level `viewX` action includes a companion `viewXs` action that returns the corresponding collection.

This version uses all your established conventions — the **Classic Story Format**, bullet-style **Data Properties** and **Actions**, proper Markdown spacing, inline links, and example values generated by default.
...
Would you like me to now update your **system prompt** so these navigation rules (showHome + viewList pairing) are part of the **Quality Gates** section?
